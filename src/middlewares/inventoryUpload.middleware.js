...
const UPLOADS_DIR = path.resolve(__dirname, '..', '..', 'uploads', 'inventory');
...
const storage = multer.diskStorage({
  ...
  filename: (req, file, cb) => {
    ...
    cb(null, `school-item-${uniqueSuffix}${safeExtension}`);
  },
});
...
function attachInventoryPhoto(req, res, next) {
  upload.single('photo')(req, res, (error) => {
    ...
    if (req.file) {
      req.body.photoUrl = `/uploads/inventory/${req.file.filename}`;
    }
